...
contract RedeemerTest is Test {
    ...
    function setUp() public {
        ...
        c = new Converter();
        // Deploy lender
        l = new Lender(Contracts.SWIVEL, Contracts.PENDLE);
        // Deploy redeemer
        r = new Redeemer(
            address(l),
    ...
    function testSwivelRedeem() public {
        ...
        // give swivel the compounding tokens
        // deal(Contracts.CDAI, Contracts.SWIVEL, startingBalance * 2);

        // approve lender to transfer principal tokens
        vm.startPrank(address(l));
        IERC20(principalToken).approve(address(r), balanceAmount);
        vm.stopPrank();

        vm.startPrank(msg.sender); 

        // execute the redemption
        bool result = r.redeem(1, Contracts.DAI, maturity);
        assertTrue(result);

        // verify that the underlying is now held by the redeemer contract
        assertEq(IERC20(Contracts.DAI).balanceOf(address(r)), balanceAmount);
    ...
    function testAPWineRedeem() public {
        ...
        uint256 index = IAPWineFutureVault(Contracts.APWINE_FUTURE_VAULT).getCurrentPeriodIndex();

        // Get the FYT address for the current period
        address fyt = IAPWineFutureVault(Contracts.APWINE_FUTURE_VAULT).getFYTofPeriod(index);

        console.log('fyt claimable', index, IERC20(fyt).balanceOf(address(l)));
        vm.startPrank(address(l));
        IERC20(fyt).approve(address(r), type(uint256).max);
        vm.stopPrank();

        vm.startPrank(address(r));
        address ibt = IAPWineFutureVault(Contracts.APWINE_FUTURE_VAULT).getIBTAddress();
        IERC20(ibt).approve(address(c), type(uint256).max);
        vm.stopPrank();
