...
contract LenderTest is Test {
    ...
    function setUp() public {
        ...
        // Deploy lender
        l = new Lender(address(sw), address(p));
        // Deploy marketplace
    ...
    function testFailInvalidPrincipal() public {
        ...
        l.lend(5, address(0), 0, 0, address(0), 0);
        ...
        l.lend(0, address(0), 0, 0, 0, 0, address(0), id);
        // Pendle
        vm.expectRevert(
            abi.encodeWithSelector(Exception.selector, 6, 0, 0, zero, zero)
        );
        l.lend(0, address(0), 0, 0, 0);
        // APWine
        vm.expectRevert(
            abi.encodeWithSelector(Exception.selector, 6, 0, 0, zero, zero)
        );
        l.lend(0, address(0), 0, 0, 0, 0, address(0), address(0));
        // Tempus
        vm.expectRevert(
            abi.encodeWithSelector(Exception.selector, 6, 0, 0, zero, zero)
        );
        l.lend(0, address(0), 0, 0, 0, 0, address(0));
        // Sense
        vm.expectRevert(
            abi.encodeWithSelector(Exception.selector, 6, 0, 0, zero, zero)
        );
        l.lend(0, address(0), 0, 0, 0, address(0), 0, address(0));
        // Notional
        vm.expectRevert(
            abi.encodeWithSelector(Exception.selector, 6, 0, 0, zero, zero)
        );
